...
| `portal/` | ✅ Complete | Portal UI React app — Vite + React 18 + TypeScript + Tailwind CSS. 25 TypeScript files. API client with full Platform API integration (apps, infra, compliance, scaffold, health). Layout (Sidebar, Header, AppShell), routing (React Router 6.28), common components (Badge, LoadingSpinner, StatusCard). **Dashboard panels (4 of 6 complete):** Applications (#79) ✅ — Argo CD apps with sync status, health, last deployed. Infrastructure (#80) ✅ — Crossplane Claims with ready/synced status, connection secrets. Compliance Score (#81) ✅ — Donut chart (Recharts) with color-coded severity (green/amber/red), violations + vulnerabilities breakdown with severity badges. Policy Violations (#82) ✅ — Scrollable table with Gatekeeper audit violations, constraint details, severity badges (red/yellow/blue), resource paths, violation messages, auto-refresh 30s. Remaining: Vulnerability Feed (#83), Security Events (#84), Scaffold form (#85). Multi-stage Dockerfile (Node 22 → Nginx 1.27-alpine). Security: non-root user, read-only rootfs, emptyDir volumes. TanStack Query for server state management. Bearer token authentication via `VITE_API_TOKEN`. Deployed at `portal.rdp.azurelaboratory.com`. |
