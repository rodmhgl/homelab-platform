...
| `portal/` | ✅ Complete | Portal UI React app — Vite + React 18 + TypeScript + Tailwind CSS. 27 TypeScript files. API client with full Platform API integration (apps, infra, compliance, scaffold, health). Layout (Sidebar, Header, AppShell), routing (React Router 6.28), common components (Badge, LoadingSpinner, StatusCard). **Dashboard panels (6 of 6 complete — CORE COMPLETE):** Applications (#79) ✅ — Argo CD apps with sync status, health, last deployed. Infrastructure (#80) ✅ — Crossplane Claims with ready/synced status, connection secrets. Compliance Score (#81) ✅ — Donut chart (Recharts) with color-coded severity (green/amber/red), violations + vulnerabilities breakdown with severity badges. Policy Violations (#82) ✅ — Scrollable table with Gatekeeper audit violations, constraint details, severity badges (red/yellow/blue), resource paths, violation messages, auto-refresh 30s. Vulnerability Feed (#83) ✅ — CVE table from Trivy scans with severity badges (red=CRITICAL/HIGH, yellow=MEDIUM/LOW), clickable CVE links to NVD, image names (truncated with tooltip), affected packages, fixed versions, summary footer (X CVEs across Y images), auto-refresh 30s. Security Events (#84) ✅ — Timeline table with Falco runtime security alerts, severity badges (red=CRITICAL/ALERT/EMERGENCY, yellow=ERROR, blue=WARNING/NOTICE, gray=DEBUG), rule names, resource paths (namespace/pod), truncated messages with hover tooltips, formatted timestamps, auto-refresh 30s. Remaining: Scaffold form (#85), AI Ops panel (#86). Multi-stage Dockerfile (Node 22 → Nginx 1.27-alpine). Security: non-root user, read-only rootfs, emptyDir volumes. TanStack Query for server state management. Bearer token authentication via `VITE_API_TOKEN`. Deployed at `portal.rdp.azurelaboratory.com`. |
...
| `cli/` | 🔨 In Progress | `rdp` CLI (Go + Cobra + Bubbletea) — Root command, config management (init/view/set), version, `status` (#66), `infra list/status/create` (#68, #69, #70), and `apps list/status/sync` (#67) complete. Interactive TUI wizards for storage/vault creation with DNS validation, Git auto-detection, GitOps commit flow. Next: delete command (#71), compliance/secrets/investigate/ask commands. |
