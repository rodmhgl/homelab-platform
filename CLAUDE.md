...
| `portal/` | ✅ Portal UI React app — Vite + React 18 + TypeScript + Tailwind CSS. 27 TypeScript files. API client with full Platform API integration. Layout (Sidebar, Header, AppShell), routing, common components. Multi-stage Dockerfile (Node 22 → Nginx 1.27-alpine). Security: non-root, read-only rootfs, emptyDir volumes. Bearer token auth via `VITE_API_TOKEN`. **Dashboard panels (6 of 6 complete — CORE COMPLETE):** Applications (#79) ✅, Infrastructure (#80) ✅, Compliance Score (#81) ✅, Policy Violations (#82) ✅, Vulnerability Feed (#83) ✅, Security Events (#84) ✅. Remaining: Scaffold form (#85), AI Ops panel (#86). Deployed: `portal.rdp.azurelaboratory.com`. |
...
| `cli/` | 🔨 rdp CLI (Go + Cobra + Bubbletea) — Root command, config management, version, `rdp status` (#66), `rdp infra list/status/create` (#68, #69, #70), `rdp apps list/status/sync` (#67) complete. Interactive TUI for storage/vault creation with DNS validation, Git auto-detection, GitOps commit flow. Pending: delete (#71), compliance (#73), secrets (#74), investigate (#75), ask (#76). |
