...
| `platform/kagent/` | ✅ Phase D — kagent v0.7.0 (wave 13). Natural language cluster queries via Agent/Task CRDs. Anthropic Claude Sonnet 4.5. Read-only RBAC (comprehensive cluster introspection). Default `platform-agent` with platform-specific instructions. ExternalSecret for Anthropic API key (shared with HolmesGPT). Three-source Argo CD pattern (CRDs + controller + Git base). Foundation for Platform API `/api/v1/agent/ask` endpoint (#53) and `rdp ask` CLI command (#76). |
