...
| `portal/` | ✅ Portal UI React app — Vite + React 18 + TypeScript + Tailwind CSS. 24 TypeScript files. API client with full Platform API integration. Layout (Sidebar, Header, AppShell), routing, common components. Multi-stage Dockerfile (Node 22 → Nginx 1.27-alpine). Security: non-root, read-only rootfs, emptyDir volumes. Bearer token auth via `VITE_API_TOKEN`. **Dashboard panels (3 of 6 complete):** Applications (#79) ✅, Infrastructure (#80) ✅, Compliance Score (#81) ✅. Remaining: Policy Violations (#82), Vulnerability Feed (#83), Security Events (#84), Scaffold form (#85). Deployed: `portal.rdp.azurelaboratory.com`. |
